...
//kiểm tra email có đúng định dạng không phải là mail có đuôi edu.vn để là thành viên để chỉnh sửa web
function ktem($email)
{
    $em="/([a-z]\.[a-z0-9]+)@(([a-z]|[a-z]+\.[e,d,u]+)+\.([a-z]{2,2}))/i";
...
if(isset($_POST['dangky'])&& $_POST['usename']!='' &&$_POST['passw']!='' && $_POST['email']!=''&&$_POST['gioitinh']!='')
{
   $user=$_POST['usename'];
   $pass=$_POST['passw'];
   $email=$_POST['email'];
   ...
   $password = md5($pass);
    ...
    if(ktem($email)==false)
    {
        $rs="Phải là Email có đuôi edu.vn";
    }
    ...
    {
    $query="INSERT INTO member(username,email,password,gioitinh)VALUES ('$user','$email','$password','$gt')";
